...
def simulate(V):
    ...
    alpha = {(a, t): 0 for a in A for t in range(T+1)}    #  t=0,...,T b/c used in output
    ...
    # Compute V_cal at T (used in LP and opt output), alpha at T (used in CSV)
    for a in A:
        alpha[a, T] = alpha[a, T - 1]
        V_cal[a, T] = I[a, T] + p_e*I_V[a, T]
        G[a, T] = 1                          # Behavior...
        if V_cal[a, T] > N[a]*v_l[a]:
            G[a, T] = 1 - (1 - g[a])/(v_u[a] - v_l[a])*(V_cal[a, T] - N[a]*v_l[a])/N[a]
                ...
                for t in range(1, T + 1):
                    csv_writer.writerow(
                        [a, t, S1[a, t].x, SV1[a, t].x, E1[a, t].x, EV1[a, t].x, I1[a, t].x,
                            IV1[a, t].x, alpha[a, t], D1[a, t].x, R1[a, t].x, W1[a, t].x, 
