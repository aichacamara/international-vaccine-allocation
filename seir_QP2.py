...
def main():
    global S0, SV0, E0, EV0, I0, IV0, W0, S1, SV1, E1, EV1, I1, IV1, H1, D1, R1, W1, V1, v, iter, V_opt, V_sim, \
        t_sim0, z, donor_deaths, tot_deaths, phase
    ...
    if not simulate_only:
        ...
        with open("./output/" +  input_file.split("/")[-1][0:-4] + ".log", "w") as fn:
            ...
            # first sim
            fn.write( "i     lambda    zNLP    deaths tot_deaths t_n    vacc by area\n")
            ...
            for a in A:
                for t in range(T):
                    V_total[a] += V_sim[a, t] 
            fn.write(f'0       0        0    {deaths_sim: 8.2f} {tot_deaths_sim: 8.2f}  {t_sim0: 6.2f}  ')
            for a in A:
            ...
            fn.write(f'{z_opt: 8.2f} {tot_deaths_opt: 8.2f}  {t_opt: 6.2f}  ')
            for a in A:
                fn.write(f'{V_total[a]: 5.0f} ')                    
            fn.write("\n\n") 
            ...
            if verbosity >= 2:
                ...
                for i in range(iter+1):
                    fn.write(f'{i: ^{2}}  {l[i]: 9.4f}  {z[i]: 8.2f} {z[i] - z_opt: 9.2f} {t_n[i]: 6.2f} ')
                    fn.write("\n")
                fn.write("\n")
